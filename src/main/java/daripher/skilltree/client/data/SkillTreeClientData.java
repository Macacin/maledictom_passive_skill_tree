...
public class SkillTreeClientData {
    ...
    public static int max_skill_points = Integer.MAX_VALUE;
    ...
    public static int getSkillPointCost(int level) {
        if (use_skill_cost_array) {
            if (level >= skill_points_costs.length) {
                return (int) (skill_points_costs[skill_points_costs.length - 1] * Math.pow(1.1, level - skill_points_costs.length + 1));  // Продолжите рост exponentially
            }
            return skill_points_costs[level];
        }
        return first_skill_cost + (last_skill_cost - first_skill_cost) * level / Integer.MAX_VALUE;  // Адаптируйте, уберите / max
    }
